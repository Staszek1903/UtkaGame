...
export(Array, PackedScene) var item_paths = []
...
func _ready():
	randomize()
	for i in item_paths:
		var proto = i.instance()
		if proto.get("item_name") != null:
			item_protos[proto.item_name] = proto
		else:
			proto.queue_free()
	if not Engine.is_editor_hint():
		for i in 10: call_deferred("spawn","Barrel")
		print("spawen barel")
...
func spawn(item_name):
	...
	var new_item = item_protos[item_name].duplicate()
